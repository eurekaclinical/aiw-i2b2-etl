...
CREATE OR REPLACE FUNCTION EUREKA.EK_DISABLE_INDEXES () RETURNS VOID AS $body$
    BEGIN
        EXECUTE 'DROP INDEX FACT_NOLOB';
        EXECUTE 'DROP INDEX FACT_PATCON_DATE_PRVD_IDX';
        EXECUTE 'DROP INDEX FACT_CNPT_PAT_ENCT_IDX';
        EXECUTE 'SET skip_unusable_indexes = true';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION 'An error was encountered - % -ERROR- %',SQLSTATE,SQLERRM;
    END;
...
CREATE OR REPLACE FUNCTION EUREKA.EK_ENABLE_INDEXES () RETURNS VOID AS $body$
    BEGIN
        EXECUTE 'REINDEX FACT_NOLOB';
        EXECUTE 'REINDEX FACT_PATCON_DATE_PRVD_IDX';
        EXECUTE 'REINDEX FACT_CNPT_PAT_ENCT_IDX';
        EXECUTE 'SET skip_unusable_indexes = false';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION 'An error was encountered - % -ERROR- %',SQLSTATE,SQLERRM;
    END;
